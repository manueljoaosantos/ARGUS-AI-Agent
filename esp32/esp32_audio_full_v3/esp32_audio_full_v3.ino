...
void writeWavHeader(uint8_t* b, int dataSize) {
  ...
  b[16] = 16; b[17]=0; b[18]=0; b[19]=0;   // Subchunk1Size
  b[20] = 1;  b[21]=0;                     // PCM
...
void loop() {
  ...
  float startThreshold = noiseFloor * 2.2;
  float endThreshold   = noiseFloor * 1.3;

  bool voiceStart = avg > startThreshold;
  bool voiceEnd   = avg > endThreshold;

  // DEBUG
  Serial.printf("avg=%.1f noise=%.1f start=%.1f\n", avg, noiseFloor, startThreshold);

  // 🎙️ START
  if (voiceStart) {

    if (voiceDetectedTime == 0) {
      voiceDetectedTime = millis();
    }

    if (millis() - voiceDetectedTime > MIN_VOICE_MS) {
