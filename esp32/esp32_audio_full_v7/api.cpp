...
{
  // 🔥 === FIX V6: evitar cortes de frase ===

  // ❌ ignora áudio demasiado curto (evita fragmentos)
  if (samples < MIN_AUDIO_SAMPLES)
  {
    Serial.printf("⚠️ Ignored short audio (%d samples)\n", samples);
    return;
  }

  // 🔥 limite máximo seguro (igual v6 ~2s)
  if (samples > 32000)
  {
    samples = 32000;
  }
  ...
  Serial.printf("📦 Sending audio (%d samples)\n", samples);
